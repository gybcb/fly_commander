...
| 目录模型 | 某目录的一页内容：文件项、排序、过滤、分页；计算每个项的显示角色 `FileVisualRole` | `DirectoryPage`, `FileItem`, `FileVisualRole` |
...
**架构原则**：`TCCore` 是 headless 纯 Swift、零 AppKit，**不直接定义颜色**。内核给每个 `FileItem` 计算一个**语义角色**（`FileVisualRole`：普通 / 目录 / 已标记 / 隐藏 / 只读 / 焦点）；AppKit 层负责「角色 → 具体 `NSColor`」映射，并据此提供浅色/深色变体。这样角色判定在内核可单测，具体色值集中在视图层、改动不碰内核。

**配色基调（混合式）**：底色跟随系统深浅色；但 TC 的标志性高亮保持不变（焦点行 navy 蓝、已标记文件蓝），以保留 TC 味道。

**具体配色（TC 经典基线）：**

| 角色 | 浅色（白底） | 说明 |
|---|---|---|
| 窗格背景 | 白（浅色）/ 深灰（深色） | 跟随系统深浅色 |
| 普通文件 | 黑 `#000000` | 深色模式反色为白 |
| 文件夹（目录） | 黑 + **加粗** | TC 灵魂特征 |
| 已标记（marked）文件 | **蓝 `#0000FF`** | TC 最标志性的特征，浅/深模式均保持 |
| 焦点行背景 | **深蓝 navy `#000080`** | 经典 TC 高亮，浅/深模式均保持 |
| 焦点行文字 | 白 | 焦点行 navy 背景上 |
| 大小列 / 修改时间列 | 灰 `#808080` | 弱化次要列 |
| 隐藏 / 系统文件 | 灰（弱化） | 半透明或灰 |
| 只读文件 | 灰（可选红） | P1 先灰 |

**待决 / 假设**：
- 深色模式下 navy `#000080` 焦点行与白字对比足够，直接沿用；若后续体验不佳再为深色微调，色值集中在视图层改。
- P1 **不做**按文件类型/扩展名上色（TC 进阶配色功能），预留 `FileVisualRole` 扩展点，P5 可加。
