...
final class MainWindowControllerTests: XCTestCase {
    // 下面两条 label 断言的变异面：删 toolbarLabelKeys 的 .connect 映射，或删
    // L10nStrings 任一表的 .toolbarRemoteConnect 行 → key 缺失/查表 nil → 对应断言红
    ...
    func testToolbarLabelsEnglish() {
        ...
        XCTAssertEqual(en[.connect], "Connect to Remote")
    ...
    func testToolbarLabelsChinese() {
        ...
        XCTAssertEqual(zh[.connect], "连接到远端")
    ...
    /// 统一「连接到远端」工具栏入口端到端锁（原 SMB/SFTP 两个入口合并后的等价锁）：
    /// 真建 MainWindowController，断言 connect 项
    /// ①出现在默认工具栏（default/allowed 列表 + delegate item 工厂都活着才可能过）、
    /// ②label 为统一文案、③action 指向 menuConnect（合并后唯一的连接入口）。
    /// 上面两条纯映射测试**锁不住**这一段：删 delegate 的 .connect case 或从
    /// default 列表删 .connect → 两条语言测试仍绿（toolbarLabelKeys 映射未动），
    ...
    func testConnectItemWiredIntoDefaultToolbar() {
        let mc = MainWindowController()
        defer { mc.window = nil }
        guard let toolbar = mc.window?.toolbar else {
            return XCTFail("工具栏未装配（setupToolbar 断了）")
        }
        let items = toolbar.items.filter { $0.itemIdentifier == .connect }
        XCTAssertEqual(items.count, 1, "统一后工具栏有且仅有一个连接入口")
        guard let item = items.first else {
            return XCTFail("默认工具栏无连接项（identifier \"connect\"）。红面含义：" +
                "①toolbarDefaultItemIdentifiers/Allowed 少了 .connect，或 " +
                "②delegate 的 .connect case 返回 nil——两种退化本测试都要红。")
        }
        XCTAssertEqual(item.label, L10n.t(.toolbarRemoteConnect),
                       "连接项 label 须为统一文案（en/zh 随语言）")
        XCTAssertEqual(item.action, #selector(MainViewController.menuConnect(_:)),
                       "连接项 action 必须直达统一的 menuConnect（合并后不再有第二个入口）")
        XCTAssertNotNil(item.image, "连接项须有图标（network）")
        // 旧第二个入口（smbConnect）须已彻底移除。
        XCTAssertFalse(toolbar.items.contains { $0.itemIdentifier.rawValue == "smbConnect" },
                       "工具栏不得残留第二个连接入口")
    }

    /// default ⊆ allowed 结构不变量（issue #4 评审 major 零锁缺口）。工具栏自定义面板
    /// 只从 allowed 列表取可拖项；若某项漏进 default 却漏进 allowed，按钮仍显示
    /// （default 生效）但用户一旦移除就再也拖不回——全测试套件（含真窗 item 锁 + UITests
    /// 只验默认项）对此全绿。故显式钉 default ⊆ allowed 且连接项同时在两表。
    /// 变异：从 toolbarAllowedItemIdentifiers 删 .connect → Set.isSubset 红。
    func testDefaultIdentifiersAreSubsetOfAllowed() {
        ...
        XCTAssertTrue(a.contains(.connect), "allowed 缺 .connect（移除后无法拖回）")
    }
}
